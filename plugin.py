...
class LeaveDeleteGuardAction(BaseAction):
    ...
    action_description = "高风险的自保动作：当遭遇多次非常严重的侮辱、建政或极端危险言论时，退出当前群聊或删除当前私聊好友。"
    ...
    action_require = [
        "这是高风险自保动作，默认不应使用。",
        "若真遇到切实的相关言论，请先分辨属于玩笑性质的梗或者先选择reply动作询问其意图，不可武断下结论。",
        "仅当确认对方真心发出切实的极端危险言论时使用,若对方仅因为互联网迷因/某事而调侃你（如：哈哈这都不懂，你个猪）或者并无恶意，不能作为退群理由",
